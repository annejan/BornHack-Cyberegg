...
//!
//! Implements the USB MSC BBB (Bulk/Bulk/Bulk) protocol over embassy-usb.
//! The host sends SCSI commands wrapped in CBW (Command Block Wrapper)
//! packets on the bulk-out endpoint; the device responds with data and/or
//! a CSW (Command Status Wrapper) on the bulk-in endpoint.
//!
//! Minimal SCSI command set for a read/write block device:
...
impl<'d, D: Driver<'d>> MscClass<'d, D> {
    ...
    async fn handle_scsi<B: BlockDevice>(
        ...
        match opcode {
            ...
            SCSI_MODE_SENSE_6 => {
                // Minimal mode sense: 4-byte header, no pages.
                let resp = [0x03, 0x00, 0x00, 0x00]; // 3 bytes follow, no WP, no block descriptor
                let len = (transfer_len as usize).min(resp.len());
                let _ = self.write_ep.write(&resp[..len]).await;
                (CSW_STATUS_PASSED, transfer_len - len as u32)
            }
            ...
            SCSI_READ_10 if direction_in => {
                ...
                for i in 0..count {
                    ...
                    // Send in max-packet-size chunks.
            ...
            SCSI_WRITE_10 if !direction_in => {
                ...
                let mut residue = transfer_len;
                for i in 0..count {
                    // Receive 512 bytes in max-packet-size chunks.
                    let mut pos = 0;
                    while pos < 512 {
            ...
            _ => {
                ...
                self.state.last_sense = SENSE_INVALID_CMD;
                // If host expects data in, we need to stall or send zeros.
